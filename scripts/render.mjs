...
function projectCards(repos) {
  ...
  return `<div class="grid" id="project-grid">${repos
    .map(
      (r) => `
      <a class="card" href="${esc(r.url)}" data-language="${esc(r.language || 'other')}" data-topics="${esc((r.topics || []).join(' '))}">
        <div class="card__top">
...
<main id="top">
  ...
  <section class="shell section" id="work">
    <div class="section__head">
      <h2>Work</h2>
      <p class="section__note">Every repository below is public, and every number is read from the GitHub API when this page is built.</p>
    </div>
    ${filterChips(repos)}
    ${projectCards(repos)}
  </section>
